            ...
            if (isinstance(response_expr, Relational) and isinstance(rubric_expr, Relational)) and response_expr.rel_op == rubric_expr.rel_op:
                if allowSimplify:
                    try:
                        ...
                        #handling TypeError: bad operand type for unary -: 'FunctionClass'
                ...
                for ky, vr in dict1.iteritems():
                    if (ky.name == par) and (vr is not None):
                        try:
                            ...
                            #--take out this contraint--if there are more than one occurrence of _ai or _mi, match is not valid
                            for atm in atmset:
                                try:
                                    if (atm.name.startswith('_a')):
                                        vr = vr.subs(atm,0)
...
def matchtestlib():
    ...
    #assert matchDouble('cos(t+pi,evaluate=False)', 'a*cos(b*t+c)+d', ['a','b','c','d'], [], ['t'])==[1.0, 1.0, 3.141592653589793, 0.0]
...
@app.route('/isequivalent', methods=['GET', 'POST'])
def checkequivalence():
    studentResponse = request.form["response"]
    exemplar = request.form["exemplar"]
    allowSimplification = request.form["simplify"] == 'True'
    trig = request.form["trig"] == 'True'
    log = request.form["log"] == 'True'
    force = request.form["force"] == 'True'
    try:
...
@app.route('/parsable', methods=['GET', 'POST'])
def checkparsable():
    studentResponse = request.form["response"]
    try:
...
@app.route('/matchexpression', methods=['GET', 'POST'])
def checkmatchexpression():
    studentresponse = request.form["response"].strip()
    pattern = request.form["pattern"].strip()
    parameters = request.form["parameters"].strip().split('|')
    constraints = request.form["constraints"].strip().split('|')
    variables = request.form["variables"].strip().split('|')
...
@app.route('/matchdouble', methods=['GET', 'POST'])

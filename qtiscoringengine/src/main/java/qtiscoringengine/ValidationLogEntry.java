...
class ValidationLogEntry {
	...
	String writeHtmlTableRow() {
		...
		return TDSStringUtils.format("<tr><td><textarea readonly rows=" + "\""
				+ numRows + "\" cols=\"125\" style=" + "\"" + "border:none;\""
				+ ">{0}</textarea></td><td>{1}</td></tr>",
	...
	private int getChildNodeCount(List<Element> childnodes)
    {
        ...
        for (Element node : childnodes)
            cnt += getChildNodeCount(node.getChildren());

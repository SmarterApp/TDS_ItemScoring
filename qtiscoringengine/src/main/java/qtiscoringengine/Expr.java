...
class ExprAnd extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    for (DataElement param : paramValues) {
      ...
      if (((DEBoolean) param).getValue () == false)
  ...
  private boolean validateTolerance (boolean ok, ValidationLog log, DEIdentifier toleranceMode, DEString tolerance, DEBoolean includeLower, DEBoolean includeUpper) {
    ...
    if (_mode.get () != ToleranceMode.Exact) {
      if ((includeLow == null) || (includeHi == null)) {
        log.addMessage (_node, "When the tolerance mode is not 'exact' you must specify includeLowerBound and includeUpperBound");
        return false;
      }
    }
    if (includeLow != null)
      _includeLowerBound = includeLow.getValue ();
    if (includeHi != null)
      _includeUpperBound = includeHi.getValue ();
    return ok;
  }

  @Override
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) throws Exception {
    ...
    default:
      throw new Exception ("ExprEquals got an unknown tolerance mode");
    }
  }
}// end class ExprEqual
...
class ExprIndex extends Expression
{
  ...
  @Override
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    DEInteger idx = (DEInteger) getAttributeValue ("index");
    if (idx == null)
      return null;
...
class ExprNot extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    return new DEBoolean (!result.getValue ());
  }
}// end class ExprNot
...
class ExprOr extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    for (DataElement param : paramValues) {
      if (((DEBoolean) param).getValue () == true)
        return new DEBoolean (true);
    }
    return new DEBoolean (false);
  }
...
class ExprPatternMatch extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    // Regex pattern = new Regex(val.Value);
    // return new DEBoolean(pattern.IsMatch(((DEString)paramValues[0]).Value));
    return new DEBoolean (((DEString) paramValues.get (0)).getValue ().matches (val.getValue ()));
  }
...
class ExprRandom extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    // int randSet = (int)Math.Truncate(randomizer.nextDouble() * 1000000);
    int randSet = (int) Math.round (randomizer.nextDouble () * 1000000 - 0.5f);
...
class ExprStringMatch extends Expression
{
  ...
  public DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    return new DEBoolean (string1.compareTo (string2) == 0);
  }
...
class ExprTruncate extends Expression
{
  ...
  protected DataElement exprEvaluate (VariableBindings vb, QTIRubric rubric, List<DataElement> paramValues) {
    ...
    return new DEInteger ((int) Math.round (((_DEFloat) paramValues.get (0)).getValue ().doubleValue () - 0.5f));
  }

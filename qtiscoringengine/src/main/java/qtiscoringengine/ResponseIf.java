...
public class ResponseIf extends ResponseRule
{
  ...
  static ResponseIf fromXml (Element node, XmlNamespaceManager nsmgr, ValidationLog log) {
    if (node == null)
      return null;
    ...
    XmlElement nodeToXml = new XmlElement (node);

    Element condNode = nodeToXml.selectSingleNode (QTIXmlConstants.ExpressionElementGroup, nsmgr);

    List<Element> ruleNodes = nodeToXml.selectNodes (QTIXmlConstants.ResponseRuleElementGroup, nsmgr);
  ...
  private DataElement evaluateResponseRules (VariableBindings vb, QTIRubric rubric) throws QTIScoringException {
    DataElement de = null;
    for (ResponseRule rule : _rules) {

...
public class ResponseSetOutcome extends ResponseRule
{
  ...
  DataElement evaluate (VariableBindings vb, QTIRubric rubric) throws QTIScoringException {
    ...
    // Make sure that the data type for the value matches the one for the
    // identifier
    // For example, an expression could produce a float but if the outcome
    // declaration is an
    // int, we need to make sure the value going into the variable is a
    // DEInteger type
    OutcomeDeclaration declaration = rubric.getOutcomeDeclaration (this._identifier);
    if (declaration != null && value != null) {
      if (value.getType () != declaration.getType ()) {
        value = DataElement.create (value.toString (), declaration.getType ());
